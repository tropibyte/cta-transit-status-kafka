...
class Station(Producer):
    ...
    def __init__(self, station_id, name, color, direction_a=None, direction_b=None):
        ...
        # Each station gets its own arrival topic. The prefix is what `consumers/server.py`
        # subscribes to with the regex `^org.chicago.cta.station.arrivals.`, so it must not
        # change. One partition and one replica is right for a single-broker cluster and for
        # the low per-station arrival rate.
        ...
        try:
            self.producer.produce(
                ...
                value={
                    ...
                    # Null on the very first arrival of each train, which is why the schema
                    # declares these two as unions with null.
            ...
            logger.error(
                "failed to produce arrival event for station %s: %s", self.name, e
            )
